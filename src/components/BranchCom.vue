      ...
      <v-row class="mx-1 body-1 grey--text text--darken-1 font-weight-black text-no-wrap" style="margin:5px auto;background:rgba(255, 255, 255, .4);">
          ...
          <div id="branchBox" class="componentBox" :style="heightOfBox">
            <div v-for="(item,index) in $store.state.dataB" :key="index">
              <v-row v-if="$store.state.peopleY > item.waitingP && $store.state.timeY > item.waitingT" style="margin:0;">
  ...
  export default {
    ...
    computed: {
      heightOfBox:function() {
        ...
        } else if(this.$store.state.inBranch) {
          that.boxHeight = that.$store.state.listHeight
        }
    ...
    watch: {
      play:function() {
        ...
        if(this.play) {
          window.scrollId3 = window.setInterval(scroll, this.$store.state.carouselTime);
        } else {
